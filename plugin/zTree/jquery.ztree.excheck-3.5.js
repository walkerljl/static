...
(function($){
	...
	_zTreeTools = function(setting, zTreeTools) {
		...
		/**
		 * 2014-6-30 huangran add. 
		 * This function is designed for the situation below, 
		 * if checkTypeFlag equals true, then whether current node is checked or not only reflect it's parents, not children.
		 */  
		zTreeTools.checkNodeOnlyParent = function(node, checked, checkTypeFlag, callbackFlag) {
			var checkedKey = setting.data.key.checked;
			if (node.chkDisabled === true) return;
			if (checked !== true && checked !== false) {
				checked = !node[checkedKey];
			...
			if (tools.uCanDo(this.setting) && setting.check.enable && node.nocheck !== true) {
				node[checkedKey] = checked;
				var checkObj = $$(node, consts.id.CHECK, setting);
				if (checkTypeFlag || setting.check.chkStyle === consts.radio.STYLE) view.checkNodeRelationOnlyParent(setting, node);
				view.setChkClass(setting, checkObj, node);
				view.repairParentChkClassWithSelf(setting, node);
				if (callbackFlag) {
					setting.treeObj.trigger(consts.event.CHECK, [null, setting.treeId, node]);
	...
	_view = {
		...
		/**
		 * 2014-6-30 huangran add. 
		 * Whether current node is checked or not only reflect it's parents, not children.
		 */  
		checkNodeRelationOnlyParent: function(setting, node) {
			var pNode, i, l,
			childKey = setting.data.key.children,
			checkedKey = setting.data.key.checked,
			r = consts.radio;
